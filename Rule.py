...
class TagAttributeMatcher(MXMLNodeMatcher):
    _fortag=None
    _forattribute=None
    def __init__(self,fortag,forattribute):
        MXMLNodeMatcher.__init__(self)
        if not(isinstance(fortag,basestring)and fortag is not None and fortag !=""):
            raise ValueError("fortag must be string value which is not none and not empty")
        if not(isinstance(forattribute,basestring)and forattribute is not None and forattribute!=""):
            raise ValueError("forattribute must be string value which is not none and not empty")
        self._fortag=fortag
        self._forattribute=forattribute
    def match(self,mxmlnode):
        return mxmlnode.getClassName()==self._fortag and mxmlnode.hasAttribute(self._forattribute)
...
class AttributeSupplyer(MXMLNodeSupplyer):
    ...
    def supply(self,mxmlnode):
        if not mxmlnode.hasAttribute(self._before):
            return
        attvalue=mxmlnode.getAttibute(self._before)
...
def makeRuleFromConfRule(rule):
    ...
    try:
        ...
        if not(validMatchTag or validMatchAttribute):
            raise ValueError("tag or attribute require valid text")
    except:
    ...
    try:
        ...
        if hasSupply and not (validTag or validAttribute or validStyle or validQuote):
    ...
    matcher=\
        TagAttributeMatcher(matchTag,matchAttribute) if validMatchTag and validMatchAttribute else\
        TagMatcher(matchTag) if validMatchTag else\
        AttributeMatcher(matchAttribute)
    tagrule=\
